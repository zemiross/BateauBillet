...
// Route canonical path (country/slug) → priority (from main sitemap, 0.4–0.9)
...
function getPageAndPriority(relPath) {
  ...
  // Route image: /images/{slug}.jpg → /{country}/{slug}
  for (const [canonical, priority] of routePriority) {
    const slug = canonical.split("/")[1];
    if (baseName === slug && !normalized.includes("/")) {
      return { loc: `${baseUrl}/${canonical}`, priority };
    }
  }
  ...
  if (
    ...
    return { loc: `${baseUrl}/navieras/balearia`, priority: 0.8 };
  }
  ...
  if (baseName.includes("balearia-regina-baltica") || baseName.includes("regina-baltica")) {
    return { loc: `${baseUrl}/navieras/balearia-regina-baltica`, priority: 0.5 };
  }
  // Info avec-voiture (0.5)
  if (baseName.includes("avec-voiture") || baseName === "bateau-avec-voiture") {
    return { loc: `${baseUrl}/info/avec-voiture`, priority: 0.5 };
  }
  // Homepage / general (0.9)
  return { loc: baseUrl, priority: 0.9 };
}

const allImages = collectImages(imagesDir);
// Group by (loc, priority) so we output one <url> per page with multiple <image:image>
const byPage = new Map();
for (const rel of allImages) {
  const { loc, priority } = getPageAndPriority(rel);
  const key = `${loc}|${priority}`;
  if (!byPage.has(key)) byPage.set(key, { loc, priority, images: [] });
  byPage.get(key).images.push(rel);
}

const urlEntries = [...byPage.entries()].sort((a, b) => {
  const [locA, priA] = [a[1].loc, a[1].priority];
  const [locB, priB] = [b[1].loc, b[1].priority];
...
for (const [, { loc, priority, images }] of urlEntries) {
...
console.log("Generated public/image-sitemap.xml with", allImages.length, "images in", byPage.size, "URLs.");
